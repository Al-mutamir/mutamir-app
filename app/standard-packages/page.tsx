...
export default function StandardPackagesPage() {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        ...
        const agencyPackages = allPackages
          .filter((pkg) => pkg.createdBy !== "admin" && !pkg.isStandard)
          .map((pkg) => ({
            ...pkg,
  ...
  const PackageCard = ({ pkg, showAgency = false }) => (
    <Card key={pkg.id} className="overflow-hidden flex flex-col h-full">
      <div className="relative h-48 w-full">
        ...
        {showAgency && pkg.agencyName && (
          <div className="absolute top-4 right-4">
            <Badge variant="secondary" className="bg-blue-100 text-blue-800">
              <Building className="h-3 w-3 mr-1" />
              {pkg.agencyName || "Unknown Agency"}
            </Badge>
          </div>
        )}
      </div>
