...
import { getPackageById, updatePackage, getUserData } from "@/lib/firebase/firestore"
import { ArrowLeft, Plus, Minus, Save, Eye, Loader2 } from "lucide-react"
...
export default function EditPackagePage() {
  ...
  useEffect(() => {
    const fetchPackage = async () => {
      try {
        ...
        if (pkg.agencyId !== user?.uid) {
          toast({
            title: "Access Denied",
            description: "You don't have permission to edit this package",
            variant: "destructive",
          })
          router.push("/dashboard/agency/offerings")
          return
        ...
        // Find the selected image based on imageUrl
        const selectedImage = packageImages.find((img) => img.url === pkg.imageUrl)?.id || ""

        // Extract service IDs from the services array
        const serviceIds = pkg.services?.map((service) => (typeof service === "string" ? service : service.id)) || []
        ...
        setPackageData({
          ...
          itinerary: pkg.itinerary || [{ dayRange: "1-3", title: "", description: "", location: "" }],
          departureDate: pkg.departureDate || "",
          arrivalDate: pkg.arrivalDate || "",
          flexibleDates: pkg.flexibleDates !== undefined ? pkg.flexibleDates : true,
        })
      } catch (error) {
  ...
  const removeItineraryPeriod = (index) => {
  ...
  const updateItineraryPeriod = (index, field, value) => {
    setPackageData((prev) => ({
      ...prev,
  ...
  const handleSubmit = async (status = "draft") => {
    ...
    try {
      ...
      const packagePayload = {
        ...
        handlerFirstName: userData?.firstName || "N/A",
        handlerLastName: userData?.lastName || "N/A",
        handlerFullName: `${userData?.firstName} ${userData?.lastName}`,
        status: status,
        ...
        exclusions: availableServices
          ...
          .map((s) => s.label),
        groupSize: packageData.type === "group-umrah" ? 30 : 20,
        ...
        departureDate: packageData.flexibleDates ? "Flexible" : packageData.departureDate,
        arrivalDate: packageData.flexibleDates ? "Flexible" : packageData.arrivalDate,
        flexibleDates: packageData.flexibleDates,
      }

      await updatePackage(id, packagePayload)

      toast({
        title: status === "active" ? "Package Updated & Published" : "Package Updated",
        description:
          status === "active"
            ? "Your package has been updated and is now visible to pilgrims"
            : "Your package has been updated and saved as a draft",
        variant: "default",
      })
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Update your Hajj or Umrah package details</p>
            </div>
          </div>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Update the basic details of your package</CardDescription>
                </CardHeader>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>How your package will appear to pilgrims</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  {packageData.selectedImage && (
